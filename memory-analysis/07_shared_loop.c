...
int main() {
    ...
    int _return_value = 0;
    long * results = rt_array_create_long(__arena_1__, 0, (long[]){});
    {
        long i = 0L;
        while (rt_lt_long(i, 3L)) {
            {
                long * temp = rt_array_create_long(__arena_1__, 3, (long[]){i, rt_add_long(i, 1L), rt_add_long(i, 2L)});
                (results = rt_array_concat_long(__arena_1__, results, temp));
            }
        __for_continue_0__:;
            rt_post_inc_long(&i);
        }
    }
    rt_print_string("Results from shared for: ");
    rt_print_array_long(results);
    rt_print_string("\n");
    long count = 0L;
    long sum = 0L;
    while (rt_lt_long(count, 5L)) {
        {
            (sum = rt_add_long(sum, count));
    ...
    ({
        ...
        _str_part0 = rt_to_string_string(__arena_1__, "Sum from shared while: ");
    ...
    ({
        char *_str_arg0 = ({
        char *_str_part0, *_str_part1, *_str_part2;
        _str_part0 = rt_to_string_string(__arena_1__, "Total from shared for-each: ");
