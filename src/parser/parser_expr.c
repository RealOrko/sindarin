...
Expr *parser_primary(Parser *parser)
{
    ...
    /* UUID.method() static call - UUID is a keyword token, not identifier */
    if (parser_match(parser, TOKEN_UUID))
    {
        Token uuid_token = parser->previous;

        /* Check for static method call: UUID.method() */
        if (parser_check(parser, TOKEN_DOT))
        {
            /* Save the type name token */
            Token type_name = uuid_token;
            type_name.start = arena_strndup(parser->arena, uuid_token.start, uuid_token.length);
        ...
        /* UUID without dot - error, not a valid expression */
        parser_error(parser, "UUID cannot be used as an expression; use UUID.method() for static calls");
        return NULL;
    }
    /* Environment.method() static call - Environment is a keyword token, not identifier */
    if (parser_match(parser, TOKEN_ENV))
    {
        Token env_token = parser->previous;

        /* Check for static method call: Environment.method() */
        if (parser_check(parser, TOKEN_DOT))
        {
            /* Save the type name token */
            Token type_name = env_token;
            type_name.start = arena_strndup(parser->arena, env_token.start, env_token.length);
        ...
        /* Environment without dot - error, not a valid expression */
        parser_error(parser, "Environment cannot be used as an expression; use Environment.get() or Environment.all()");
        return NULL;
    }
    if (parser_match(parser, TOKEN_LEFT_PAREN))
    {
